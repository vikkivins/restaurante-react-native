...
export default function Porcoes({ navigation }) {
  ...
  const fetchPorcoes = async () => {
    try {
      // Dados do JSON local
      const localData = gourmetData.porcoes || [];

      // Dados do Firebase
      const db = firebase.firestore();
      const snapshot = await db.collection('produtos')
        .where('categoria', '==', 'Porcoes')
        .get();
      ...
      setPorcoes([...localData, ...firebaseData]);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = navigation.addListener('focus', () => {
      fetchPorcoes(); // Recarrega os dados
    });
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setPorcoes(prevPorcoes => prevPorcoes.filter(item => item.id !== id));
    } catch (error) {
